...
// ============================================================================
// X64CodeGenerator — Module generation
// ============================================================================
...
void X64CodeGenerator::emit_module_header(const ir::Module& module) {
    ...
    // Build set of locally-defined function names (non-empty bodies).
    std::set<std::string> local_fns;
    for (const auto& func : module.functions) {
        if (!func->blocks.empty()) {
            local_fns.insert(func->name);
        }
    }

    // Static set of runtime functions called via hardcoded opcodes in codegen
    // (these are not declared as IR functions by the IR generator).
    static const char* k_hardcoded_runtime[] = {
        "golangc_println_int",   "golangc_println_string", "golangc_println_bool",
        "golangc_println_float",
        "golangc_print_int",     "golangc_print_string",   "golangc_print_bool",
        "golangc_print_float",   "golangc_print_space",    "golangc_print_newline",
        "golangc_panic",
        "golangc_string_concat", "golangc_string_eq",
        "golangc_select",
        "golangc_chan_make",      "golangc_chan_send",       "golangc_chan_recv",
        "golangc_go_spawn",
        "golangc_map_make",      "golangc_map_get",         "golangc_map_set",
    ...
    // Emit hardcoded runtime EXTERNs (skip locally-defined names).
    for (const char* name : k_hardcoded_runtime) {
        if (local_fns.count(name) == 0 && emitted.count(name) == 0) {
            out_ += fmt::format("EXTERN {}:PROC\n", name);
            emitted.insert(name);
        }
    }

    // Emit dynamic runtime EXTERNs (IR functions with no body).
    for (const auto& func : module.functions) {
        if (func->blocks.empty() && emitted.count(func->name) == 0) {
            out_ += fmt::format("EXTERN {}:PROC\n", func->name);
            emitted.insert(func->name);
        }
    }

    // Always-needed non-function externals
...
void X64CodeGenerator::emit_function(const ir::Function& func) {
    // Skip external declarations (runtime functions with no body).
